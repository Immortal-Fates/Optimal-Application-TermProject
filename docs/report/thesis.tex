...
在 $G=4$ 的场景下，枚举规模随 $B$ 的增长近似为 $O(B^{G-1})$，对本文设定的 batch 范围而言完全可接受。相比启发式调度，exact 枚举具有确定性、可解释性强的优势。

\subsection{工程实现与工具链}

系统实现基于 PyTorch DDP 与 Hugging Face Transformers/Datasets\cite{HFTransformers,HFDatasets}，数据集为 GLUE/SST-2\cite{GLUEBenchmark}，模型为 DistilBERT\cite{DistilBERT}。训练过程记录 per-step 指标并输出到 CSV，分析脚本基于 pandas 与 matplotlib 自动生成图表与汇总表。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/nvidia-smi-before.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/nvidia-smi-after.png}
...
\item 训练配置：4 GPU，global batch $B=48$，steps=800，max\_length=256
\item Baseline：static（每卡固定 12）
...
表\ref{tab:summary} 汇总了静态与动态调度的对比结果。动态调度在时延和吞吐上均有显著改善：mean max step time 降低 17.13\%，P95 降低 17.19\%；samples/s 平均提升 20.68\%，P95 提升 20.58\%。更显著的是，跨卡 padded tokens 标准差降低 70.06\%，同时 peak memory 标准差降低 56.91\%，表明动态调度在负载均衡与显存稳定性上均取得明显收益。
...
\hline
Mean max step time (ms) & 118.51 & 98.20 & -17.13 \\
P95 max step time (ms) & 120.96 & 100.17 & -17.19 \\
Mean throughput (samples/s) & 405.10 & 488.85 & +20.68 \\
P95 throughput (samples/s) & 413.39 & 498.47 & +20.58 \\
Mean throughput (tokens/s) & 13302.11 & 15610.87 & +17.36 \\
P95 throughput (tokens/s) & 16549.53 & 19757.56 & +19.38 \\
Mean std(step time) & 0.177 & 0.134 & -24.26 \\
Mean std(padded tokens) & 87.69 & 26.25 & -70.06 \\
Mean std(peak mem) & 1.489 & 0.642 & -56.91 \\
\hline
\end{tabular}
\end{table}
