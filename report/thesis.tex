...
\]
该代理具有单调性，并与注意力计算复杂度一致性较好\cite{Vaswani2017}。在工程实现中，计算成本与显存开销可用同一代理近似，从而降低建模复杂度。
...
\begin{enumerate}
\item 每个 rank 预取 $K$ 个样本形成 buffer，计算 $c_g(k)$；
\item 通过 all-gather 将成本数组汇总到 rank0；
...
实验在单机 4 卡环境上完成，GPU 为 4 张 NVIDIA RTX 3090。图\ref{fig:nsmi1} 和图\ref{fig:nsmi2} 为实验前后的 nvidia-smi 截图，用于展示硬件配置与 GPU 占用情况。

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/nvidia-smi1.png}
\caption{实验环境 nvidia-smi 截图（1）}
\label{fig:nsmi1}
\end{figure}
